...
public class DialogueVertexAnimator {
    ...
    public IEnumerator AnimateTextIn(List<DialogueCommand> commands, string processedMessage, Action onFinish) {
        textAnimating = true;
        float secondsPerCharacter = 1f / 40f;
    ...
    // Clear the dialogue mesh
    public void Clear_DialogueBox()
    {
        TMP_TextInfo textInfo = textBox.textInfo;
        for (int i = 0; i < textInfo.meshInfo.Length; i++) //Clear the mesh 
        {
            ...
            if (command.position == visableCharacterIndex) {
                switch (command.type) {
                    ...
                    case DialogueCommandType.TextSpeedChange:
                        ...
                        break;
                    case DialogueCommandType.React:
                        SwitchOff_Animation_State();
                        if (command.stringValue != "isNone")
                            anim.SetBool(command.stringValue, true);
                        break;
                }
                commands.RemoveAt(i);
                i--;
    ...
    private const float NOISE_MAGNITUDE_ADJUSTMENT = 0.24f;
    private const float NOISE_FREQUENCY_ADJUSTMENT = 15f;
    private const float WAVE_MAGNITUDE_ADJUSTMENT = 0.24f;
    private Vector3 GetAnimPosAdjustment(TextAnimInfo[] textAnimInfo, int charIndex, float fontSize, float time) {
        ...
        for (int i = 0; i < textAnimInfo.Length; i++) {
            ...
            if (charIndex >= info.startIndex && charIndex < info.endIndex) {
                ...
                } else if (info.type == TextAnimationType.circle) {
                    y -= Mathf.Sin((time * 6)) * fontSize * WAVE_MAGNITUDE_ADJUSTMENT;
                    x += Mathf.Sin(1.5f + (time * 6)) * fontSize * WAVE_MAGNITUDE_ADJUSTMENT;
                }
            }
        }
    ...
    public void SwitchOff_Animation_State()
    {
