...
public class DialogueUtility : MonoBehaviour
{
    ...
    private const string REACT_REGEX_STRING = "<rt:(?<react>" + REMAINDER_REGEX + ")>";
    private static readonly Regex reactRegex = new Regex(REACT_REGEX_STRING);
    ...
    private static readonly Dictionary<string, string> reactDictionary = new Dictionary<string, string>{
        { "None", "isNone" },
        { "Scared", "isScared" },
        { "Angry", "isAngry" },
    };
    ...
    public static List<DialogueCommand> ProcessInputString(string message, out string processedMessage)
    {
        ...
        processedMessage = HandleReactTags(processedMessage, result);
    ...
    private static string HandleReactTags(string processedMessage, List<DialogueCommand> result)
    {
        MatchCollection reactMatches = reactRegex.Matches(processedMessage);
        foreach (Match match in reactMatches)
        {
            string stringVal = match.Groups["react"].Value;
            string reactName = stringVal;
            Debug.Assert(reactDictionary.ContainsKey(reactName), "no react registered for '" + reactName + "'");
            result.Add(new DialogueCommand
            {
                position = VisibleCharactersUpToIndex(processedMessage, match.Index),
                type = DialogueCommandType.React,
                stringValue = reactDictionary[reactName]
            });
        }
        processedMessage = Regex.Replace(processedMessage, REACT_REGEX_STRING, "");
        return processedMessage;
    }
